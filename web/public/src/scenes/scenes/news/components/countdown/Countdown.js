  ...
  render() {
    ...
    return (
      <div className={prefixClass}>
        ...
        <span className={`${prefixClass}-hours`}>{('0' + hours).slice(-2)}</span>
        <span className={`${prefixClass}-minutes`}>{('0' + minutes).slice(-2)}</span>
        <span className={`${prefixClass}-seconds`}>{('0' + seconds).slice(-2)}</span>
      </div>
    )
  }
}
