  ...
  renderHints() {
    ...
    return (
      <div>
        ...
        <ul>
          {hints.map((hint, i) => (
            <li key={i}><ReactMarkdown source={hint.body} /></li>
          ))}
        </ul>
      </div>
    )
      ...
      >
        <div className={className}>
          <div className={`${prefixClass}-info`}>
            ...
            <div className={`${prefixClass}-difficulty`}>{challenge.difficulty}</div>
          ...
          {user.isSolved
              ? <div className={`${prefixClass}-solved-message`}>Challenge solved.</div>
              : (
